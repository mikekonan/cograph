...
async def test_live_postgres_graph_api_serves_list_and_detail(
    ...
    async with integration_session_manager.session() as session:
        repository = Repository(
            ...
            # Endpoints are queried anonymously below; public_read only
            # reaches PUBLIC repos.
...
""",
    ...
    repo_path = f"{repository.host}/{repository.owner}/{repository.name}"
    list_response = await integration_client.get(
        f"/api/repos/{repo_path}/graph",
        params={"view": "symbols"},
    )
    detail_response = await integration_client.get(
        f"/api/repos/{repo_path}/graph/nodes/{helper_node.id}"
    )
