"""BankDocumentEmbedderService — embeds bank_document_chunks incrementally.
        ...
        chunks = list(
            (
                await session.scalars(
                    ...
                    .join(BankDocument, BankDocumentChunk.document_id == BankDocument.id)
                    .where(BankDocument.bank_id == bank_id)
                )
            ).all()
        )
            ...
            (
                await session.scalars(
                    select(BankDocumentChunk).where(
                        BankDocumentChunk.document_id.in_(document_ids)
                    )
                )
            ).all()
        )
        ...
        for start in range(0, len(to_embed), self._batch_size):
            ...
            for (chunk_id, _, computed_hash), vector in zip(batch, vectors, strict=True):
                await session.execute(
                    update(BankDocumentChunk)
                    .where(BankDocumentChunk.id == chunk_id)
                    .values(
                        embedding=vector,
...
def _chunk_text(chunk: BankDocumentChunk) -> str:
    parts: list[str] = []
