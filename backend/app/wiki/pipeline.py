    ...
    *,
    ...
    if mode == "full":
        ...
        if incremental_enabled:
            ...
            #
            # Spec carry-over: the fresh plan is regenerated by the (non-
            # deterministic) planner, so an unchanged page's contract — title,
            # covers_questions, tree position — gets reworded run-to-run,
            # firing `spec_changed` and rewriting a page whose code never moved.
            # That is the dominant steady-state cost once retrieval jitter is
            ...
            prior_specs = (
                {spec.slug: spec for spec in rehydrated_prior.plan.pages}
                if rehydrated_prior is not None
                else {}
            )
            report = await compute_dirty_slugs(
                session,
            ...
            if report.reconciled_specs:
                plan = plan.model_copy(
                    update={
                        "pages": [
                            report.reconciled_specs.get(page_spec.slug, page_spec)
