...
# Deliberately NOT served over MCP: the whole-repo graph snapshot
# (`.../graph`, up to 1000 nodes + edges ≈ 40-60k tokens) and the per-node
# graph resource (`.../graph/node/{id}`, a duplicate of cograph_read_node).
# Nothing in the playbook ever recommended them; agents traverse the graph
# through the capped cograph_related tool instead. The web UI keeps its own
